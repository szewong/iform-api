...
const getAPIPath =(path)=>{
    if(path[0]!="/") path = "/" + path ;
    return `/${API_GATEWAY_PATH}/${_getLocal(SERVERNAME_KEY)}/api` + path;
}
...
const  withAuthenticator = (WrappedComponent, ifbConfig) => {
    ...
    const authenticate = ()=> {
        sendAPI(`/${API_GATEWAY_PATH}/${servername}/authenticate?redirect_uri=${redirect_uri}`,'get').then (response => {
    ...
    const token = (code)=>{
        ...
        sendAPI(`/${API_GATEWAY_PATH}/${servername}/token`,'post',body).then (response => {
            if (response.token){
